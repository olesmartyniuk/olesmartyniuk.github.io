...
Я пам'ятаю випадок, коли деякі тести почали падати в перший ж день місяця: програміст розраховував, що в місяці завжди буде як мінімум 30 днів. У лютому його припущення не справдилося. 
...
Ця помилка теж не дуже поширена. Але інколи буває, що програмісти тримають тести до коду, який вони написали, лише на своєму ПК, або тести знаходяться в репозиторії та їх необхідно запускати вручну. Або запуск тестів автоматизовано, але тригером є певна подія, ініційована людиною: наприклад, лідер команди QA натискає кнопку у веб-інтерфейсі.
...
Наразі є технології віртуалізації, що спрощують цей процес. Запуск `Docker` контейнерів займає менше секунди. Тобто, менше секунди часу і ви маєте БД з підготовленими для ваших тестів даними. Наслідком цього є те, що вам необхідно підтримувати цей скрипт. Він повинен стати частиною репозиторію і змінюватись разом зі зміною структури даних.
...
Уявіть, що у нас метод, що видаляє файл. Він перевіряє права доступу, далі звертається до сервісу файлів і логує результат операції. Всі сервіси, використані в цьому методі, є зовнішніми залежностями, функціонал яких ми не будемо тестувати. В коді тесту вони будуть існувати у вигляді моків.
...
Якщо програміст каже, що не тестує те, що не важливо, можна запитати, для чого він додав неважливий код взагалі? Часто програмісти ігнорують перевірку запису логів. Логи програми, можливо, не важливі для програміста зараз і їх навряд чи можна назвати частиною бізнес логіки. Але логи точно стануть важивими, коли код не буде працювати у користувача. Коли логи - єдине джерело інформації для знаходження дефекту, вони надзвичайно важливі.

> Не варто плутати "неповні тести" і "неповне покриття тестами". Неповне покриття означає, що деякі ділянки коду, або деякі шляхи виконання програми не задіяні в тестах. Це може бути припустимим, якщо це якісь унікальні випадки, кількість можливих тестових сценаріїв завелика і ваша команда не прагне до 100 % покриття.
