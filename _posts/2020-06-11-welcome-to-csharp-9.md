...
Ключове слово `data` у визначенні класу позначає клас як запис. Це додає йому певної поведінки характерної для типу-значення, яку ми розглянемо далі. Загалом, записи краще розглядати як "значення" (дані), а не як об'єкти. Вони не створені, щоб мати змінюваний стан. Натомість, ви виражаєте зміни у часі, створюючи нові записи, що зображають новий стан. Вони визначаються не унікальністю посилання, а тотожністю вмісту.

### Вираз with
При роботі з незмінними даними, загальний підхід полягає у створенні копії для зображення нового стану об'єкту. Для прикладу, якщо наша особа захоче змінити своє прізвище, ми реалізуємо це через створення нового об'єкту, що буде копією старого окрім іншого прізвища. Цю техніку часто називають *неруйнівною мутацією*. Замість того, щоб змінювати особу *з часом* запис зображує стан особи в *конкретний момент часу*.  
...
Це означає те, що згідно з їхньою "значимістю", два об'єкти-записи можуть бути рівними, будучи *різними* екземплярами одного типу. Для прикладу, якщо ми повернемо назад прізвище у раніше зміненої особи:  
...
Якщо вам не підходить порівняння по полях, що визначається за умовчанням, ви можете написати своє. Але треба бути обережним і розуміти, як працює порівняння за значеннями в структурах, особливо якщо є наслідування (до якого ми ще повернемося нижче).   
...
Напевно, є допустимі приклади використання змінюваного стану записів, зокрема для кешування. Але ручна робота, необхідна для того, щоб перевизначити поведінку так, щоб ігнорувати цей стан, імовірно, буде доволі значною.
...
І знову, C# подбав про це. Спосіб, в який це реалізовано: записи мають віртуальну захищену властивість `EqualityContract`. Кожен похідний запис перевизначає її, та для того, щоб два записи були однаковими, вони повинні мати один і той самий `EqualityContract`.
...
int? result = b ? 0 : null; // значущий nullable тип
```
